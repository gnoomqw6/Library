...
function validateForm() {
    ...
    if (pass == null || pass == "" || pass.match(/^\s+$/)) {
        alert("Пароль обязателен для заполнения. Он не может быть пустым или состоять только из пробелов.");
        return false;
    }
    ...
    if (username == null || username == "" || username.match(/^\s+$/)) {
        alert("Нам необходимо знать, как к Вам обращаться, поэтому имя обязательно для заполнения. " +
